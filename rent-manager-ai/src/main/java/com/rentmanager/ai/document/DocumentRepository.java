...
public final class DocumentRepository {
    ...
    public DocumentRepository(final JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public List<DocumentMetadata> list(final DocumentScope scope) {
        return jdbc.query("""
                ...
                """, DocumentRepository::map, scope.propertyId(), scope.contractId());
    }
    ...
    public Optional<DocumentMetadata> find(final DocumentScope scope, final UUID id, final boolean lock) {
        final String sql = "SELECT * FROM documents WHERE property_id = ? AND contract_id = ? AND document_id = ?"
                + (lock ? " FOR UPDATE" : "");
        return jdbc.query(sql, DocumentRepository::map, scope.propertyId(), scope.contractId(), id).stream().findFirst();
    }
