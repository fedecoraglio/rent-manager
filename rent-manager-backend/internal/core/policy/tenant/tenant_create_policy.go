...
type TenantCreatePolicy struct {
	tenantRepository port.TenantRepository
}

func NewTenantCreatePolicy(tenantRepository port.TenantRepository) *TenantCreatePolicy {
	return &TenantCreatePolicy{
		tenantRepository: tenantRepository,
	}
}

func (tenantPolicy *TenantCreatePolicy) Execute(ctx context.Context, tenant *domain.Tenant) error {
	if tenant == nil {
	...
	existingTenant, err := tenantPolicy.tenantRepository.GetTenantByDocumentNumber(ctx, tenant.DocumentNumber)
	if err != nil {
